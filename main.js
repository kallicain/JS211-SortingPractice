  ...
  const sumEvens = stringsToNumbs.filter(num => num % 2 == 0).reduce((a, c) => a + c);

  console.log("Sum evens",sumEvens);
  ...
  //using a higher order function, create an array of the unique 'weather_state_name' values of the weather array. Your function should return the following array ['Light Cloud', 'Heavy Cloud', 'Showers']
   //const map1 = array1.map(x => x * 2);
     // const result = words.filter(word => word.length > 6);
  // const weatherStates = weather.map(name => name.weather_state_name).filter((name, i, arr) => arr.indexOf(name) == i);
  const weatherStates = weather.map(name => name.weather_state_name)
  const noDuplicates = [... new Set(weatherStates)];
